...
func doAuth() error {
	...
	err = copyFileFromTemplate("templates/views/login.jet", cel.RootPath+"/mail/login.jet")
	if err != nil {
		exitGracefully(err)
	}

	err = copyFileFromTemplate("templates/views/forgot.jet", cel.RootPath+"/mail/forgot.jet")
	if err != nil {
		exitGracefully(err)
	}

	err = copyFileFromTemplate("templates/views/reset-password.jet", cel.RootPath+"/mail/reset-password.jet")
	if err != nil {
		exitGracefully(err)
	}

	color.Yellow("  - users, token, and remember_tokens migrations created and executed")
